		...
		public async Task<ActionResult> TotalFactors(int? Id, string txtdate1, string txtdate2, int? IsAcceptByAdmin)
		{
			...
			if (txtdate1 != null && txtdate2 != null && txtdate1 != "" && txtdate2 != "")
			{
				...
				if (!DateTime.TryParse(txtdate1, out dt))
				{
					...
					{
						_totalPrice2 += item.OriginalPrice;
					}
		...
		public ActionResult ShowSubsets(int? Id, string searchstring, string txtdate1, string txtdate2, int? IsAdminCheck)
		{
			...
			if (txtdate1 != null && txtdate2 != null && txtdate1 != "" && txtdate2 != "")
			{
				...
				if ((DateTime.TryParse(txtdate1, out dt) && DateTime.TryParse(txtdate2, out dt)) != true)
				{
					...
					foreach (var item in db.MarketerFactor.Where(p => p.MarketerUser.Parent_Id == Id).ToList())
					{
						_TotalSumFactor += item.OriginalPrice;
				...
				{
					foreach (var item in db.MarketerFactor.Where(p => p.MarketerUser.Parent_Id == Id).Where(p => p.Date >= dateTime1 && p.Date <= dateTime2 && p.IsAdminCheck).ToList())
					{
						_Totalsumfactors += item.OriginalPrice;
				...
				foreach (var item in db.MarketerFactor.Where(p => p.MarketerUser.Parent_Id == Id).Where(p => p.Date >= dateTime1 && p.Date <= dateTime2).ToList())
				{
					_Totalsumfactors += item.OriginalPrice;
				}
				ViewData["TotalSumFactor"] = _Totalsumfactors;
			...
			if (IsAdminCheck == 1 && IsAdminCheck != null)
			{
				...
				foreach (var item in db.MarketerFactor.Where(p => p.MarketerUser.Parent_Id == Id).Where(s => s.IsAdminCheck).ToList())
				{
					TotalSumFactor1 += item.OriginalPrice;
				}
			...
			var TotalFactors = db.MarketerFactor.Where(p => p.MarketerUser.Parent_Id == Id).ToList();
			double TotalSumFactor = 0;
			foreach (var item in TotalFactors)
			{
				TotalSumFactor += item.OriginalPrice;
			}
